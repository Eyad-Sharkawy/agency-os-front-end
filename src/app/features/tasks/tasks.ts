  ...
  template: `
    <div class="space-y-6">
      <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 class="text-ink font-display text-2xl font-bold tracking-tight">Tasks</h1>
          <p class="text-body-muted text-sm">
            Manage agile workstreams, sprints, and task assignments across projects.
          </p>
        </div>
        <aos-button variant="primary" size="sm">
          <aos-icons name="lucidePlus" class="size-4" />
          <span class="ml-1.5 font-mono text-xs">Create Task</span>
        </aos-button>
      </div>
      ...
      <div class="border-hairline bg-canvas rounded-md border p-12 text-center">
        ...
        >
          <aos-icons name="lucideCheckSquare" class="size-6" />
        </div>
        <h3 class="text-ink mt-4 text-base font-semibold">Task Boards Ready</h3>
        <p class="text-body-muted mx-auto mt-1 max-w-sm text-xs">
          Task status workflows (TODO, IN_PROGRESS, REVIEW, DONE) are configured.
        </p>
      </div>
    </div>
  `,
