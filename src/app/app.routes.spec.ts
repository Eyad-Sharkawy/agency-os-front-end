...
describe("App Routes Configuration", () => {
  it("should have correct route paths defined", () => {
    ...
    // Demo showcase route
    const demoRoute = routes[2];
    expect(demoRoute.path).toBe("demo");
    expect(typeof demoRoute.loadComponent).toBe("function");

    // Workspaces feature routes
    const workspacesRoute = routes[3];
    ...
    const unauthorizedRoute = routes[4];
    ...
    const workspaceShellRoute = routes[5];
