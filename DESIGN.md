...
Large brand green taxonomy chip used on the blog index. Active chips invert to brand green fill with dark text; inactive chips use brand green outline and pale fill. Typography is oversized relative to typical filters, making the taxonomy a hero-level control.
...
Dark footer subscription block with brand green "AI moves fast" label, white headline, muted legal microcopy, a single-line email field, and arrow submit marker. Footer columns use white section labels and muted links.
...
- Use brand green for identity taxonomy and brand accents, not as an overpowering decorative fill.
